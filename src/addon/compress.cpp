...
Napi::Value Compress(const Napi::CallbackInfo &info) {
  ...
  if (bzCompressStatus != BZ_OK) {
    if (bzCompressStatus == BZ_CONFIG_ERROR) {
      ...
      Napi::Error::New(info.Env(), "Compress not reached").ThrowAsJavaScriptException();
    }
  ...
  TransformStream.Set("_transform", Napi::Function::New(info.Env(), [&](const Napi::CallbackInfo &info) {
    ...
    if (!(bzCompressStatus == BZ_RUN_OK || bzCompressStatus == BZ_OK)) {
      ...
      else return Callback.Call({Napi::Error::New(info.Env(), "Compress not reached").Value()});
    }
  ...
  // set final stream
  TransformStream.Set("_final", Napi::Function::New(info.Env(), [&](const Napi::CallbackInfo &info) {
