...
static Bool isempty_RL(EState* s) {
  if (s->state_in_ch < 256 && s->state_in_len > 0) return False;
  else return True;
}
...
int BZ_API(BZ2_bzCompressInit)(bz_stream* strm, int blockSize100k, int verbosity, int workFactor) {
  ...
  if (strm->bzalloc == NULL || !strm->bzalloc) strm->bzalloc = default_bzalloc;
  if (strm->bzfree == NULL || !strm->bzfree) strm->bzfree = default_bzfree;
  EState* s = (EState*)BZALLOC(sizeof(EState));
...
static void add_pair_to_block ( EState* s ) {
  ...
  for (i = 0; i < s->state_in_len; i++) BZ_UPDATE_CRC(s->blockCRC, ch);
...
static Bool handle_compress(bz_stream* strm) {
  Bool progress_in  = False, progress_out = False;
  ...
  while (true) {
    ...
    if (s->state == BZ_S_INPUT) {
        ...
        if (s->mode != BZ_M_RUNNING && s->avail_in_expect == 0) {
          ...
          s->state = BZ_S_OUTPUT;
        } else if (s->strm->avail_in == 0) break;
    }
...
int BZ_API(BZ2_bzCompress)(bz_stream *strm, BZ_Action action) {
  Bool progress;
  if (strm == NULL) return BZ_PARAM_ERROR;
  EState* s = (EState*)strm->state;
  if (s == NULL) return BZ_PARAM_ERROR;
  if (s->strm != strm) s->strm = strm;
  ...
  switch (s->mode) {
    ...
    case BZ_M_RUNNING:
      if (action == BZ_RUN) return handle_compress(strm) ? BZ_RUN_OK : BZ_PARAM_ERROR;
               ...
               int     len )
{
   ...
   while (True) {
      ...
      ret = BZ2_bzCompress ( &(bzf->strm), BZ_Action::Run);
      if (ret != BZ_RUN_OK)
                    ...
                    unsigned int* nbytes_out_hi32 )
{
   ...
   if ((!abandon) && bzf->lastErr == BZ_OK) {
      while (True) {
         bzf->strm.avail_out = BZ_MAX_UNUSED;
         bzf->strm.next_out = bzf->buf;
         ret = BZ2_bzCompress ( &(bzf->strm), BZ_Action::Finish );
                           ...
                           int           workFactor )
{
   ...
   ret = BZ2_bzCompress ( &strm, BZ_Action::Finish );
